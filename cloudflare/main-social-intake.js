...
async function supervisorSession(request,env){
  ...
  const row=await env.DB.prepare("SELECT user_label FROM telecaller_sessions WHERE token=? AND active=1").bind(token).first();
  if(!row||!SUPERVISORS.has(row.user_label))return null;
...
async function intakeInstagram(env,body){
  ...
  if(sourceType==='comment'&&commentId){
    ...
    if(!existing)existing=await env.DB.prepare(`SELECT id,whatsapp_prefill_token,assigned_to,interested_area FROM social_leads WHERE platform='instagram' AND platform_user_id=? AND COALESCE(source_media_id,'')=COALESCE(?, '') AND keyword=? ORDER BY id DESC LIMIT 1`).bind(platformUserId,mediaId,match.keyword).first();
  }else existing=await env.DB.prepare(`SELECT id,whatsapp_prefill_token,assigned_to,interested_area FROM social_leads WHERE platform='instagram' AND platform_user_id=? AND COALESCE(source_media_id,'')=COALESCE(?, '') AND keyword=? ORDER BY id DESC LIMIT 1`).bind(platformUserId,mediaId,match.keyword).first();
...
async function summary(env,dateValue=null){
  ...
  const byArea=(await env.DB.prepare(`SELECT COALESCE(sl.interested_area,'Not selected') AS area,COUNT(DISTINCT sl.id) AS enquiries,COUNT(DISTINCT CASE WHEN se.event_type='landing_area_selected' THEN sl.id END) AS selected,COUNT(DISTINCT CASE WHEN se.event_type IN ('whatsapp_number_captured','instagram_phone_captured') THEN sl.id END) AS captured FROM social_leads sl LEFT JOIN social_lead_events se ON se.social_lead_id=sl.id AND date(datetime(se.created_at,'+5 hours','+30 minutes'))=? WHERE sl.platform='instagram' AND sl.keyword='AREA' AND date(datetime(sl.created_at,'+5 hours','+30 minutes'))<=? GROUP BY COALESCE(sl.interested_area,'Not selected') HAVING selected>0 OR captured>0 OR (area='Not selected' AND enquiries>0) ORDER BY captured DESC,selected DESC,area ASC`).bind(date,date).all()).results||[];
  return {ok:true,date,timezone:'Asia/Kolkata',metrics:{area_comments:Number(commentsRow?.count||0),area_selected:Number(selectedRow?.count||0),whatsapp_opened:Number(selectedRow?.count||0),crm_leads_captured:Number(capturedRow?.count||0)},by_area:byArea};
}

async function socialDetails(request,env){
  const viewer=await supervisorSession(request,env);if(!viewer)return {error:'Supervisor session required',status:401};
  const url=new URL(request.url),date=validDate(url.searchParams.get('date'));
  if(!date)return {error:'Valid date required',status:422};
  const stage=String(url.searchParams.get('stage')||'comments').toLowerCase();
  const area=String(url.searchParams.get('area')||'').trim();
  let condition="date(datetime(sl.created_at,'+5 hours','+30 minutes'))=?";
  if(stage==='selected')condition="EXISTS (SELECT 1 FROM social_lead_events sx WHERE sx.social_lead_id=sl.id AND sx.event_type='landing_area_selected' AND date(datetime(sx.created_at,'+5 hours','+30 minutes'))=?)";
  if(stage==='captured')condition="EXISTS (SELECT 1 FROM social_lead_events sx WHERE sx.social_lead_id=sl.id AND sx.event_type IN ('whatsapp_number_captured','instagram_phone_captured') AND date(datetime(sx.created_at,'+5 hours','+30 minutes'))=?)";
  const params=[date];let areaSql='';if(area){areaSql=' AND sl.interested_area=?';params.push(area);}
  const rows=(await env.DB.prepare(`SELECT sl.id,sl.platform_username,sl.interested_area,sl.whatsapp_phone,sl.lead_id,sl.assigned_to,sl.status,sl.created_at,sl.updated_at,l.display_phone,l.phone,l.name,l.lead_code,l.status AS crm_status FROM social_leads sl LEFT JOIN leads l ON l.id=sl.lead_id WHERE sl.platform='instagram' AND sl.keyword='AREA' AND ${condition}${areaSql} ORDER BY sl.updated_at DESC,sl.id DESC LIMIT 250`).bind(...params).all()).results||[];
  return {ok:true,date,stage,area:area||null,viewer:viewer.user_label,leads:rows.map(r=>({...r,phone:r.display_phone||r.whatsapp_phone||r.phone||null,name:r.name||null}))};
}

export default {async fetch(request,env,ctx){
  const url=new URL(request.url);const path=url.pathname.replace(/\/$/,'')||'/';
  if(request.method==='OPTIONS')return new Response(null,{status:204,headers:corsHeaders(env)});
  if(request.method==='POST'&&path==='/api/social/instagram/intake'){const body=await request.json().catch(()=>({}));const result=await intakeInstagram(env,body);return json(result,result.status||200,env);}
  if(request.method==='POST'&&path==='/api/social/area/select'){const body=await request.json().catch(()=>({}));const result=await selectAreaFromLanding(env,body);return json(result,result.status||200,env);}
  if(request.method==='GET'&&path==='/api/social/area/go'){const result=await selectAreaFromLanding(env,{ref:url.searchParams.get('ref'),area:url.searchParams.get('area')});if(!result.ok)return new Response(result.error||'Unable to continue',{status:result.status||400,headers:{'content-type':'text/plain; charset=utf-8','cache-control':'no-store'}});return Response.redirect(result.whatsapp_url,302);}
  if(request.method==='GET'&&path==='/api/social/summary')return json(await summary(env,url.searchParams.get('date')),200,env);
  if(request.method==='GET'&&path==='/api/social/leads'){const result=await socialDetails(request,env);return json(result,result.status||200,env);}
