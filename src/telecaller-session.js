...
async function sendActivity(){if(!session||document.hidden)return;const now=Date.now();if(now-lastActivitySent<45000)return;lastActivitySent=now;try{await api("/api/session/activity",{method:"POST",body:"{}"});}catch(_){} }
function startChecks(){if(checkTimer)clearInterval(checkTimer);if(hoursTimer)clearInterval(hoursTimer);checkTimer=setInterval(()=>{renderStrip();if(session)document.body.classList.toggle("tc-session-active",isTelecaller(session.user_label));},60000);hoursTimer=setInterval(()=>{if(session&&isSupervisor(session.role))loadWorkHours();},120000);}
...
async function callEnd(){if(!session||!callWasStarted)return;callWasStarted=false;try{await api("/api/session/call-end",{method:"POST",body:"{}"});}catch(_){} }
