...
function ensureStrip(){
  ...
  root.insertAdjacentHTML("afterend",`<div id="tc-session-strip"><span>● Logged in: <b id="tc-session-user"></b></span><span>ID <b id="tc-session-id"></b></span><span>Role <b id="tc-session-role"></b></span><span>Started <b id="tc-session-start"></b></span><span>Active <b id="tc-session-elapsed"></b></span><span>Idle logout: <b>10 min</b></span><button id="tc-logout">Logout</button></div>`);
...
function hideGate(){ensureUI();document.querySelector("#staff-login-gate").classList.remove("show");document.body.classList.add("staff-session-active");document.body.classList.toggle("tc-session-active",isTelecaller(session?.user_label));ensureStrip();renderStrip();}
function renderStrip(){if(!session)return;ensureStrip();const u=document.querySelector("#tc-session-user"),id=document.querySelector("#tc-session-id"),r=document.querySelector("#tc-session-role"),s=document.querySelector("#tc-session-start"),e=document.querySelector("#tc-session-elapsed");if(u)u.textContent=session.user_label;if(id)id.textContent=session.employee_id||"—";if(r)r.textContent=session.role||"staff";if(s)s.textContent=fmtTime(session.login_at);if(e)e.textContent=elapsed(session.login_at);}
...
async function logout(reason="manual"){
  ...
  localStorage.removeItem(TOKEN_KEY);session=null;document.body.classList.remove("staff-session-active","tc-session-active");showGate();window.dispatchEvent(new CustomEvent("crm-session-logout",{detail:{reason}}));
}
...
async function restore(){
  ...
  try{session=await api("/api/session/me");localStorage.setItem(USER_KEY,session.user_label);syncSelector(session.user_label);hideGate();startChecks();}
...
function startChecks(){if(checkTimer)clearInterval(checkTimer);checkTimer=setInterval(async()=>{renderStrip();if(!session)return;try{session={...session,...await api("/api/session/me")};document.body.classList.toggle("tc-session-active",isTelecaller(session.user_label));}catch(e){if(e.status===401)logout("idle");}},30000);}
...
window.addEventListener("crm-user-changed",e=>{if(session&&e.detail?.user!==session.user_label)syncSelector(session.user_label);});
window.CRM_SESSION={api,token:()=>localStorage.getItem(TOKEN_KEY)||"",user:()=>session?.user_label||localStorage.getItem(USER_KEY),role:()=>session?.role||null,isActive:()=>!!session,restore,logout,sendActivity};
